...
def train_mlp(train_loader, test_loader, text_input_size, image_input_size, output_size, num_epochs=50, report=False, lr=0.001, set_weights=True, adam=False, p=0.0, seed=1, patience=40, save_results=True, train_model=True, test_mlp_model=True):
    ...
    model = create_early_fusion_model(text_input_size, image_input_size, output_size, hidden=[1024, 512], p=p)
    ...
    history = None
    if train_model:
        history = model.fit(train_loader, epochs=num_epochs, validation_data=test_loader, class_weight=class_weights, callbacks=[early_stopping])
    ...
    if test_mlp_model:
        ...
        for batch in test_loader:
            ...
            if len(features) == 1:
                    if 'text' in features:
                        preds = model.predict(features['text'], verbose=0)
                    else:
                        preds = model.predict(features['image'], verbose=0)
    ...
    # Diagnostic block only runs when training was performed (history exists)
    if train_model:
        print("="*50)
        print("DIAGNOSTIC")
        print("="*50)
        final_val_acc = history.history['val_accuracy'][-1]
        print(f"Keras validation accuracy (last epoch): {final_val_acc:.4f}")
        manual_val_loss, manual_val_acc = model.evaluate(test_loader, verbose=0)
        print(f"Manual model.evaluate() on test_loader: {manual_val_acc:.4f}")
        print(f"Difference: {abs(final_val_acc - manual_val_acc):.4f}")
