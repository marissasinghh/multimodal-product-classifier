...
def train_and_evaluate_model(X_train, X_test, y_train, y_test, models=None, test=True, class_names=None):
    """
    ...
    # If class_names provided, use them for visualization
    y_test_display = class_names if class_names is not None else y_test
    
    visualize_embeddings(X_train, X_test, y_train, y_test_display, plot_type='2D', method='PCA')
    ...
    if not(models):
        ...
        try:
            ...
            models.append(('XGBoost', XGBClassifier(n_estimators=100, learning_rate=0.1, random_state=42, eval_metric='mlogloss')))
        except ImportError:
            pass
        try:
